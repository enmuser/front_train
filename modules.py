...
class ConvNeXt_block(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        input = x # x = 16 * 640 * 16 * 16, time_emb = 16 * 64
        time_emb = self.mlp(time_emb) #  time_emb = 16 * 64 ==> time_emb = 16 * 640
        x = self.dwconv(x) +  rearrange(time_emb, 'b c -> b c 1 1') # # self.dwconv(x) =16 * 640 * 16 * 16 + rearrange(time_emb, 'b c -> b c 1 1') = 16 * 640 * 1 * 1  ==> x = 16 * 640 * 16 * 16
        x = x.permute(0, 2, 3, 1) # (N, C, H, W) -> (N, H, W, C) # # x = 16 * 640 * 16 * 16 ==> x = 16 * 16 * 16 * 640
        x = self.norm(x)
        x = self.pwconv1(x) #  #  x = 16 * 16 * 16 * 640 ==>  x = 16 * 16 * 16 * 2560
        x = self.act(x)
        x = self.pwconv2(x) # x = 16 * 16 * 16 * 2560 ==> x = 16 * 16 * 16 * 640
        if self.gamma is not None:
            x = self.gamma * x # self.gamma = (640,) ==> x = 16 * 16 * 16 * 640
        x = x.permute(0, 3, 1, 2) # (N, H, W, C) -> (N, C, H, W) # #  x = 16 * 16 * 16 * 640  ==> 16 * 640 * 16 * 16

        x = input + self.drop_path(x)
        return x # x =  16 * 640 * 16 * 16
...
class ConvNeXt_bottle(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        input = x # input = x  = 16 * 1280 * 16 * 16 , time_emb = 16 * 64
        time_emb = self.mlp(time_emb) #  time_emb = 16 * 64 ==> time_emb = 16 * 640
        x = self.dwconv(x) + rearrange(time_emb, 'b c -> b c 1 1') # self.dwconv(x) =16 * 640 * 16 * 16 + rearrange(time_emb, 'b c -> b c 1 1') = 16 * 640 * 1 * 1  ==> x = 16 * 640 * 16 * 16
        x = x.permute(0, 2, 3, 1) # (N, C, H, W) -> (N, H, W, C) # x = 16 * 640 * 16 * 16 ==> x = 16 * 16 * 16 * 640
        x = self.norm(x)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        shorcut = x.clone() # shorcut = 16 * 64 * 64 * 64, x = 16 * 64 * 64 * 64
        x = self.proj_1(x) #  x = 16 * 64 * 64 * 64
        x = self.activation(x) #
        x = self.spatial_gating_unit(x) # x = 16 * 64 * 64 * 64
        x = self.proj_2(x)
        x = x + shorcut
        return x
